...
CREATE TABLE `user` (
  ...
  `new_customer_status` tinyint(1) NOT NULL DEFAULT '0' COMMENT '首次付费状态：0=未触发，1=已触发（首次付费成功后设为1，用于邀请奖励幂等）',
...
CREATE TABLE `job` (
  ...
  `address` varchar(512) DEFAULT NULL COMMENT '岗位地址（区域）',
  `address_detail` varchar(512) DEFAULT NULL COMMENT '岗位详细地址',
  ...
  `create_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `update_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  `refresh_time` datetime(3) DEFAULT NULL COMMENT '免费刷新时间',
  `paid_refresh_time` datetime(3) DEFAULT NULL COMMENT '付费刷新时间',
